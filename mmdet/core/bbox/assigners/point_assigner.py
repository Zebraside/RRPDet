...
def get_center_point(x, y):
    return torch.cat([torch.mean(x, axis=-1, keepdim=True), torch.mean(y, axis=-1, keepdim=True)], dim=1)
...
class PointAssigner(BaseAssigner):
    ...
    def assign(self, points, gt_bboxes, gt_bboxes_ignore=None, gt_labels=None):
        ...
        if gt_bboxes.shape[1] == 5:
            gt_bboxes_xy = torch.tensor(gt_bboxes[:, :2]).cuda()
            gt_bboxes_wh = torch.tensor(gt_bboxes[:, 2:4]).cuda()
        else:
        ...
        for idx in range(num_gts):
            ...
            assigned_gt_dist[min_dist_points_index] = min_dist[
                less_than_recorded_index].cuda().float()
